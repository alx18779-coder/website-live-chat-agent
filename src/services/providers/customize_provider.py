...
class CustomizeLLMProvider(LLMProvider):
    """自定义 LLM提供商"""
...
class CustomizeEmbeddings(Embeddings):
    """自定义自定义Embedding类，确保发送文本而不是token ID数组"""

    def __init__(self, api_key: str, model: str = "BAAI/bge-large-zh-v1.5", base_url: str = "https://api.customize.cn/v1"):
    ...
    async def _make_embedding_request(self, input_data, is_query: bool = False):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if status_code >= 500:
                    # 5xx 错误，可以重试
                    logger.warning(f"Customize API 5xx 错误 (尝试 {attempt + 1}/{self.max_retries + 1}): {e}")
                    if attempt < self.max_retries:
                        await asyncio.sleep(self.retry_delay * (2 ** attempt))  # 指数退避
                        continue
                else:
                    # 4xx 错误，不重试
                    logger.error(f"Customize API 4xx 错误: {e}")
                    raise

            except (httpx.TimeoutException, httpx.ConnectError, httpx.NetworkError) as e:
                last_exception = e
                logger.warning(f"Customize API 网络错误 (尝试 {attempt + 1}/{self.max_retries + 1}): {e}")
                if attempt < self.max_retries:
                    await asyncio.sleep(self.retry_delay * (2 ** attempt))  # 指数退避
                    continue
                else:
                ...
                last_exception = e
                logger.error(f"Customize API 未知错误: {e}")
                raise

        # 如果所有重试都失败了
        if last_exception:
            logger.error(f"Customize API 重试失败，已尝试 {self.max_retries + 1} 次")
...
class CustomizeEmbeddingProvider(EmbeddingProvider):
    """自定义 Embedding提供商"""

    def get_required_config_fields(self) -> List[str]:
        return ["api_key"]  # base_url现在是可选的

    def create_embeddings(self) -> CustomizeEmbeddings:
        """创建自定义 Embeddings实例（使用自定义实现）"""
        return CustomizeEmbeddings(
            api_key=self.config["api_key"],
            model=self.config.get("model", "BAAI/bge-large-zh-v1.5"),
            base_url=self.config.get("base_url", "https://api.customize.cn/v1")
        )

    def get_models(self) -> List[str]:
        """获取支持的自定义 Embedding模型列表"""
        return [
